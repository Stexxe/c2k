...
//func convert(sc *Scope, file *KtFile) {
//	imports := map[*Fqn]struct{}{}
//
//	for fqn := range sc.Imports {
//		file.ImportList = append(file.ImportList, Import{fqn: fqn})
//	}
//
//	for _, decl := range sc.FuncDecls {
...
//		}
//
//		for fqn := range bodyScope.Imports {
//			file.ImportList = append(file.ImportList, Import{fqn: fqn})
//		}
//
//		if len(bodyScope.Statements) == 1 {
//			if r, ok := bodyScope.Statements[0].(*returnStat); ok {
//				astDecl.Expr = convertExpr(file, r.expr)
//			}
//		}
//
...
//}
//
//func convertExpr(file *KtFile, in any) (out any) {
//	switch expr := in.(type) {
//	case funcCall:
//		call := FuncCall{Name: symbolName(expr.signature.fqn)}
//		var args []any
//
//		for _, a := range expr.args {
//			args = append(args, convertExpr(file, a))
//		}
//
//		call.ValueArgs = args
//		out = call
...
//FuncCall{Name: runBlocking.Name, ValueArgs: []any{
...
//	fileScope.TopScope = fileScope
//
//	declareFunc(fileScope, mainSign, func(sc *Scope) {
//		ret(sc, runBlockingCall(sc, func(sc *Scope) {
//			declareVal(sc, "client", httpClientType, callCtor(sc, httpClientType.class))
//		}))
//	})
//
//	return
...
//	declareFunc(fileScope, mainSign, func(sc *Scope) {
//		ret(sc, runBlockingCall(sc, func(sc *Scope) {
//			declareVal(sc, "client", httpClientType, ctorCall{httpClientType.class})
//		}))
//	})
//
//	//convert(fileScope, file)
//	return
//
//	//file = &KtFile{}
//	//builderFound := false
